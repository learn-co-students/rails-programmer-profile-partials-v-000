...
describe "the attribute partial" do
  let(:programmer) { FactoryGirl.create(:programmer) }
  programmers_show = File.open("app/views/programmers/show.html.erb", "r").read
  ...
  it "renders any attribute of the programmer" do
      ...
      expect(programmers_show).to include("<%= render partial: 'claim_to_fame', locals: {programmer: @programmer} %>")
